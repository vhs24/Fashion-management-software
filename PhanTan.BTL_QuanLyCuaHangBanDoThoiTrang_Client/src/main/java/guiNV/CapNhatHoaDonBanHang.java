...
public class CapNhatHoaDonBanHang extends JFrame {
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					try {
						...
						capnhatSanPham();
						if (rs) {
							JOptionPane.showMessageDialog(rootPane, "Cập nhật hóa đơn thành công");
						} else
							JOptionPane.showMessageDialog(rootPane, "Cập nhật hóa đơn không thành công");
		...
		txtTienKhachDua.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (tienTraLai < 0) {
					...
					lblTongTien.setText("");
					// JOptionPane.showMessageDialog(null, "Số tiền mà khách hàng trả không đủ");
				} else {
	...
	public void layMaHDBH(String ma, Date ngayLap, String maKH, String maNV, double tienKhachDua) {
		...
		try {
			...
			KhachHang khach = new KhachHang(kh.getMaKhachHang(), kh.getTenKhachHang(), kh.getSoCMND(), kh.getSoDienThoai(), kh.getDiaChi(), kh.getThanhPho(), kh.getNgaySinh(),kh.isGioiTinh());
			System.out.println(kh);
			NhanVienBanHang nv = new NhanVienBanHang(maNV);
