...
public class ThemKhachHang extends JFrame {
	...
	public ThemKhachHang() {
		GenerateKeyDAO generateKeyDAO = null;
		String maKH = "1";
		try {
			generateKeyDAO = (GenerateKeyDAO) Naming.lookup("rmi://localhost:9999/generateKey");
			maKH = generateKeyDAO.getKey("KhachHang");
				...
				if (validData()) {
					...
					if(ngaySinh != null) {
						try {
							...
							KhachHang kh = new KhachHang(taoMa(), tenKH, cmnd, sdt, diaChi,
									thanhPho, ngaySinh, gioiTinh);
		...
		btnNewButton.setBounds(32, 524, 211, 50);
		...
		btnThot.setBounds(277, 524, 210, 50);
		...
		lblLoi.setBounds(10, 484, 477, 30);
