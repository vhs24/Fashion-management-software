...
public class CapNhatHoaDonNhapKho extends JFrame {
	...
	private JLabel lblThongBaoThemLoaiSP;
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					CapNhatHoaDonNhapKho frame = new CapNhatHoaDonNhapKho();
	 ...
	 */
	public CapNhatHoaDonNhapKho() throws RemoteException {
		...
		JLabel lblNewLabel_13 = new JLabel("Cập nhật hóa đơn nhập kho");
		lblNewLabel_13.setFont(new Font("Tahoma", Font.PLAIN, 40));
		lblNewLabel_13.setBounds(20, 21, 518, 60);
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					if (rs) {
						...
						lblThongBaoThemLoaiSP.setText("Thêm mới thành công loại sản phẩm");
		
					} else
						//JOptionPane.showMessageDialog(null, "Thêm mới loại sản phẩm không thành công");
						lblThongBaoThemLoaiSP.setText("Thêm mới loại sản phẩm không thành công");
				} catch (Exception e1) {
					// TODO Auto-generated catch block
					e1.printStackTrace();
				}
		...
		lblThongBaoThemLoaiSP = new JLabel("");
		lblThongBaoThemLoaiSP.setForeground(new Color(0, 0, 0));
		lblThongBaoThemLoaiSP.setFont(new Font("Tahoma", Font.PLAIN, 11));
		lblThongBaoThemLoaiSP.setBounds(10, 184, 246, 13);
		pNhapLoaiSP.add(lblThongBaoThemLoaiSP);

		//-----------------------------------------------------------------------------------
		//panel dùng nhập thêm hãng sản xuất
		pNhapHSX = new JPanel();
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					if (rs) {
						...
						lblThongBao.setText("Thêm nhà sản xuất không thành công");
				} catch (Exception e1) {
					// TODO Auto-generated catch block
					e1.printStackTrace();
				}
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					try {
						...
						boolean rs = hoaDonNhapKhoDAO.capnhatHoaDon(hdnk);
	...
	public void showTableCTHDBH(String ma) {
		...
		int num = 1;
		List<ChiTietHoaDonNhapKho> listCTHDNK;
		try {
			KhachHangDAO khachHangDAO = (KhachHangDAO) Naming.lookup("rmi://localhost:9999/khachhang");
			ChiTietBanHangDAO chiTietBanHangDAO = (ChiTietBanHangDAO) Naming.lookup("rmi://localhost:9999/chitietbanhang");
			HoaDonBanHangDAO hoaDonBanHangDAO = (HoaDonBanHangDAO) Naming.lookup("rmi://localhost:9999/hoadonbanhang");
			...
			HangSanXuatDAO hsxDAO = (HangSanXuatDAO) Naming.lookup("rmi://localhost:9999/hangsanxuat");
			LoaiSanPhamDAO loaispDAO = (LoaiSanPhamDAO) Naming.lookup("rmi://localhost:9999/loaisanpham");
			listCTHDNK = chiTietNhapKhoDAO.layDanhSachCTHoaDonNhapKhoTheoMaHDBH(ma);
			double tongTien = 0;
			for (ChiTietHoaDonNhapKho cthdbh : listCTHDNK) {
				SanPham sp = sanphamDAO.laySanPhamTheoMa(cthdbh.getSanPham().getMaSanPham());
				HangSanXuat hsx = hsxDAO.layHangSanXuatTheoMa(sp.getHangSanXuat().getMaHangSanXuat());
				LoaiSanPham loai = loaispDAO.layLoaiSanPhamTheoMa(sp.getLoaiSanPham().getMaLoaiSanPham());
				
				double thanhTien = cthdbh.getSoLuong() * cthdbh.getGiaNhap();
				model.addRow(new Object[] { num, sp.getMaSanPham(), sp.getTenSanPham(), hsx.getTenHangSanXuat(), 
						 ...
						 cthdbh.getSoLuong(), cthdbh.getGiaNhap(),
						thanhTien });
				num++;
				tongTien += thanhTien;
			}
	...
	public void layMaHDKho(String ma, Date ngayLap, String maNV) {
		//ChiTietHoaDonBanHang ctbh = (ChiTietHoaDonBanHang) chiTietBanHangDAO.layDanhSachCTHoaDonBanHangTheoMaHDBH(ma);
		try {
			KhachHangDAO khachHangDAO = (KhachHangDAO) Naming.lookup("rmi://localhost:9999/khachhang");
			ChiTietBanHangDAO chiTietBanHangDAO = (ChiTietBanHangDAO) Naming.lookup("rmi://localhost:9999/chitietbanhang");
			HoaDonBanHangDAO hoaDonBanHangDAO = (HoaDonBanHangDAO) Naming.lookup("rmi://localhost:9999/hoadonbanhang");
			...
			LoaiSanPhamDAO loaispDAO = (LoaiSanPhamDAO) Naming.lookup("rmi://localhost:9999/loaisanpham");
			ChuCuaHang cch = chucuahangDAO.layThongTinChuCuaHang();
			ChuCuaHang chu = new ChuCuaHang("CCH001");
			...
			HoaDonNhapKho hdnk = hoaDonNhapKhoDAO.layHoaDonNhapKhoTheoMa(ma);
			HoaDonNhapKho nhapKho = new HoaDonNhapKho(ma, ngayLap, nhanVien, chu);
			lblMaHD.setText(ma);
